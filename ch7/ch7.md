...
当成员定义在类的外部的时候，上述的查找中，第三步不仅要考虑类定义之前的全局作用域还需要考虑成员函数定义之前的全局作用域。

### 构造函数再探

#### 构造函数初始值列表

如果没有在构造函数的初始化列表中显示的执行初始化成员，则该成员在构造函数体之前执行默认初始化。在很多类中，初始化和赋值的区别主要是效率问题，前者直接初始化数据成员，后者先初始化再赋值。

##### 构造函数的初始值有时必不可少

如果成员是const、引用或者某种未提供默认构造函数的类类型，必须通过构造函数初始化列表为这些成员进行初始化。

##### 成员初始化的顺序

成员初始化的顺序与他们在类定义中出现顺序一致，与初始化值列表中的顺序无关。但是，如果一个成员是用另一个成员来初始化，则这两个成员的初始化顺序很关键。例如：
```
class X {
    int i;
    int j;
...
最好令构造函数初始值的顺序与成员声明的顺序保持一致，尽量避免使用一个值去初始化另一个值。

##### 默认实参和构造函数
如果一个构造函数为所有的参数都提供了默认实参，则实际上也就定义了默认的构造函数。

#### 委托构造函数
委托构造函数使用类的其他构造函数执行自己初始化的过程。此时先执行受委托的构造函数的初始化列表，然后执行其函数体，最后返回到委托构造函数。

#### 默认构造函数的作用

对象被默认初始化或者值初始化时自动执行默认构造函数。

- 默认初始化情况
    - 块作用域内不适用任何初始值定义一个非静态变量
    - 类本身含有类类型的成员且使用合成的默认构造函数
    - 类类型成员没有在构造函数初始化列表中显示初始化时

- 值初始化情况
    - 数组初始化过程中初始值少于数组的大小
    - 不适用初始值定义一个局部静态变量
    - 形如T()的表达式显示的请求值初始化，T为类型名，使用一个T类型的实参对它的元素初始化器进行值初始化

##### 使用默认构造函数
```
Sales_data obj(); //声明一个函数而不是对象
Sales_data obj2;  //obj2是一个对象而不是一个函数
```
##### 注意
C++中struct没有默认构造函数，需要自定义构造函数

#### 隐式的类类型转换
能通过一个实例调用的构造函数定义了一条从构造函数的参数类型向类类型隐式转换的规则，转换的过程中会生成一个临时对象（如果函数参数是引用类型则不可以进行隐式转换），函数执行结束之后，该临时对象就变的不可访问。

##### 只允许一步类型转换
编译器只会自动执行一步类型转换
```
item.combine("xxxxxx"); //错误
1. 把"xxxxxx"转化成string
2. 把string转化成Sales_data
```

##### 类类型转换不总是有效

##### 抑制构造函数定义的隐式转换
关键字`explicit`只对一个实参的构造函数有效，因为需要多个实参的构造函数不能用于执行隐式转换，所以，不需要将构造函数指定为`explicit`。并且，只能在类内声明构造函数时使用`explicit`关键字，在类外部的定义时不应该重复。

##### explicit构造函数只能用于直接初始化
当我们要使用 `explicit`声明的构造函数的时候，只能以直接初始化使用，而且，编译器将不会再自动转换过程中使用该构造函数
```
Sales_data item1(book); //直接初始化
Sales_data item2 = book;
```

##### 为转换显示地使用构造函数
编译器不会将`explicit`的构造函数用于隐式转换过程，但是可以使用这样的构造函数进行显示地强制转换，例如：
```
item.combine(Sales_data(book)); //显示构造
item.combine(static_cast<Sales_data>(cin)); //static_cast可以使用explicit的构造函数创建临时对象
```

##### 标准库中含有显示构造函数的类

单参数的构造函数：
- 单参数`const char *`的`string`构造函数
- 容量参数的`vector`的构造函数
