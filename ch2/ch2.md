...
- 当auto是引用类型的时候，初始值中的顶层const保留
- 在一条语句中声明多个变量，auto表示的基本类型必须保持一致

```
int i = 0, &r = i;
const int ci = i, &cr = ci;
//auto中一般会忽略掉顶层const，同时底层const则会保留下来
auto c = cr;     //c 是整型(cr是ci的别名，ci是顶层const)
auto d = &i;     //d是一个整型指针
auto e = &ci;    //e是一个整数常量的指针
const auto f = ci;     //f是const int属于顶层const
...
- 如果decltype的操作数是一个变量，则返回该变量的类型（包括顶层const和引用）

#### decltype和引用

- decltype((variable))的结果永远是引用类型，而decltyp()结果只有variable本身是引用时才是引用类型
- decltype中操作数进行解指针操作，则得到的是引用类型
```
int i = 42, *p = &i, &r = i;
...
####注意
- 定义一个类时没有相关的运算操作，可以定义成struct类型
- 对类内的变量进行初始化的时候，可以使用花括号或者放在等号右边，不可以使用圆括号
