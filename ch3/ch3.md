...
使下标访问数组，下标的类型通常为`size_t`类型，`size_t`是一种机器相关的无符号类型。当遍历数组的时候，可以使用如下的方式：
```
unsigned scores[11] = {};//不使用{}初始化会有垃圾值
for (auto i : scores) {
    cout << i << endl;
}
```
...
- 使用auto声明变量，推断得到的类型是指针非数组
```
int ia[] = {0, 1, 2, 3};
auto ia2(ia);//ia2是指针类型，指向ia的第一个元素
```
使用`decltype`关键字时不会发生上述的问题，例如：
```
...
```
int ia[] = {0, 1, 2, 3};
int *beg = begin(ia); // ia数组的首元素指针
int *bend = end(ia);  // ia数组尾元素的下一位置指针
```
- 标准库类型string或者vector限定使用的必须是无符号类型，内置的下标类型可以为**负数**

### 与旧代码接口

#### 混用string对象和C风格字符串

- 可以使用以空字符结束的字符数组来初始化string类型或者为string类型赋值
- 在string对象的加法操作中允许使用以以空字符结束的字符数组作为其中一个运算对象
...
使用for循环处理多维数组，除了内层循环之外，其他所有外层循环变量也都应该是**引用类型**
...
```
因为**row不是引用类型，编译器初始化row的时候会将其设置为指向该数组内首元素的指针，这样row的类型是int *类型不能对其进行遍历**
